...
describe('favorites model', () => {
  ...
  beforeEach((done) => {
    nightmare.goto('http://localhost').wait('body').then(done);
  });
...
describe('favorites tab', () => {
  ...
  it('changes to edit mode when "edit" is clicked', (done) => {
    nightmare
    .click('#favorites .link.edit')
    .wait(() => $('#favorites .link.done:visible').length > 1)
    .then(() => nightmare.exists('#favorites .page.edit-mode'))
  ...
  it('changes back to normal mode when "done" is clicked', (done) => {
    ...
    .click('#favorites .link.edit')
    .wait(() => $('#favorites .link.done:visible').length > 1)
    .click('#favorites .link.done')
    .wait(() => $('#favorites .link.edit:visible').length > 1)
    .then(() => nightmare.exists('#favorites .page:not(.edit-mode)'))
    .then(result => expect(result).toBeTruthy())
    .catch(util.logError)
    .then(done);
  });
